...
***************************************************************************** */
/* 
 * V0.0: 03.04.2015,	AM:  Initial Version
 * V0.1: 15.09.2016,	DW:	Adjustment for MPU9250
  */
/* *****************************************************************************
**                      Includes                                              **
***************************************************************************** */
...
void MPU9250_Setup(void)
{
	...
	uint8_t c = I2Cdev_readByte(MPU9x50_USIC,MPU9250_ADDRESS, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9150

	if (c == 0x68 || c == 0x71 || c == 0x73) // WHO_AM_I should always be 0x68 (MPU9150 or MPU9250)
	{
		if (MPU9250_SelfTest())
		{
			//MPU9250_Calibrate(); // Calibrate gyro and accelerometers, load biases in bias registers
			delay(1000);
			MPU9250_Init(); // Inititalize and configure accelerometer and gyroscope
...
{
	// wake up device
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, PWR_MGMT_1, 0x00); // Clear sleep mode bit (6), enable all sensors
	delay(100); // Delay 100 ms for PLL to get established on x-axis gyro; should check for PLL ready interrupt

	// get stable time source
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, PWR_MGMT_1, 0x01);  // Set clock source to be PLL with x-axis gyroscope reference, bits 2:0 = 001
	delay(200);
	...
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, CONFIG, 0x01);
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, CONFIG, 0x06);

	// Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, SMPLRT_DIV, 0x01);  // Use a 200 Hz rate; the same rate set in CONFIG above

	// Set gyroscope full scale range
	// Range selects FS_SEL and AFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
	//uint8_t c =  I2Cdev_readByte(MPU9x50_USIC,MPU9250_ADDRESS, GYRO_CONFIG);
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, GYRO_CONFIG, c & ~0xE0); // Clear self-test bits [7:5]
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, GYRO_CONFIG, c & ~0x18); // Clear AFS bits [4:3]
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, GYRO_CONFIG, c | Gscale << 3); // Set full scale range for the gyro
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, GYRO_CONFIG, Gscale << 3); // Set full scale range for the gyro

	// Set accelerometer configuration
	//c =  I2Cdev_readByte(MPU9x50_USIC,MPU9250_ADDRESS, ACCEL_CONFIG);
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ACCEL_CONFIG, c & ~0xE0); // Clear self-test bits [7:5]
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ACCEL_CONFIG, c & ~0x18); // Clear AFS bits [4:3]
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ACCEL_CONFIG, c | Ascale << 3); // Set full scale range for the accelerometer
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ACCEL_CONFIG, Ascale << 3); // Set full scale range for the accelerometer

	// Configure FIFO
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, INT_ENABLE, 0x00); // Disable all interrupts
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_EN, 0x00);    // Disable FIFO
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, USER_CTRL, 0x02);  // Reset I2C master and FIFO and DMP
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, USER_CTRL, 0x00);  // Disable FIFO
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, INT_PIN_CFG, 0x02);
	delay(100);

	// Read the WHO_AM_I register of the magnetometer, this is a good test of communication
	//c = I2Cdev_readByte(MPU9x50_USIC,AK8975A_ADDRESS, WHO_AM_I_AK8975A);  // Read WHO_AM_I register for AK8975A
	//if (c == 0x48)
	//	MPU9250_InitAK8975A(magCalibration);

	// Configure Magnetometer for FIFO
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_EN, 0x78); // Enable all sensors for FIFO
	/*I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_EN, 0x79); // Enable all sensors for FIFO
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_MST_CTRL, 0x5D);
	// Set up auxilliary communication with AK8975A for FIFO read
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_SLV0_ADDR, 0x8C); // Enable and read address (0x0C) of the AK8975A
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_SLV0_REG, 0x02);  // Register within AK8975A from which to start data read
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_SLV0_CTRL, 0xD7); // Read six bytes and swap bytes
	// Initialize AK8975A for write
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_SLV1_ADDR, 0x0C);  // Write address of AK8975A
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_SLV1_REG, 0x0A);   // Register from within the AK8975 to which to write
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_SLV1_DO, 0x01);    // Register that holds output data written into Slave 1 when in write mode
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_SLV1_CTRL, 0x81);  // Enable Slave 1
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_SLV4_CTRL, 0x04);*/

	// Configure Interrupts and Bypass Enable
	// Set interrupt pin active high, push-pull, and clear on read of INT_STATUS, enable I2C_BYPASS_EN so additional chips
	// can join the I2C bus and all can be controlled by the Arduino as master
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, INT_PIN_CFG, 0x00);
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, INT_ENABLE, 0x11);  // Enable data ready (bit 0) interrupt
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, INT_ENABLE, 0x01);  // Enable data ready (bit 0) interrupt

	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_MST_DELAY_CTRL, 0x83); // Enable delay of external sensor data until all data registers have been read
	//I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, USER_CTRL, 0x64);
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, USER_CTRL, 0x44);
 ...
 */
void MPU9250_InitAK8975A(float * destination)
{
...
void MPU9250_Calibrate(void)
{
	...
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, PWR_MGMT_1, 0x80); // Write a one to bit 7 reset bit; toggle reset device
	delay(100);

	// get stable time source
	// Set clock source to be PLL with x-axis gyroscope reference, bits 2:0 = 001
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, PWR_MGMT_1, 0x01);
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, PWR_MGMT_2, 0x00);
	delay(200);

	// Configure device for bias calculation
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, INT_ENABLE, 0x00);   // Disable all interrupts
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_EN, 0x00);      // Disable FIFO
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, PWR_MGMT_1, 0x00);   // Turn on internal clock source
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, I2C_MST_CTRL, 0x00); // Disable I2C master
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, USER_CTRL, 0x00);    // Disable FIFO and I2C master modes
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, USER_CTRL, 0x0C);    // Reset FIFO and DMP
	delay(15);

	// Configure MPU6050 gyro and accelerometer for bias calculation
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, CONFIG, 0x01);      // Set low-pass filter to 188 Hz
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, SMPLRT_DIV, 0x00);  // Set sample rate to 1 kHz
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, GYRO_CONFIG, 0x00);  // Set gyro full-scale to 250 degrees per second, maximum sensitivity
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ACCEL_CONFIG, 0x00); // Set accelerometer full-scale to 2 g, maximum sensitivity

	uint16_t  accelsensitivity = 16384;  // = 16384 LSB/g

	// Configure FIFO to capture accelerometer and gyro data for bias calculation
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, USER_CTRL, 0x40);   // Enable FIFO
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_EN, 0x78);     // Enable gyro and accelerometer sensors for FIFO  (max size 1024 bytes in MPU-6050)
	delay(80); // accumulate 80 samples in 80 milliseconds = 960 bytes

	// At end of sample accumulation, turn off FIFO sensor read
	I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_EN, 0x00);        // Disable gyro and accelerometer sensors for FIFO
	I2Cdev_readBytes(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_COUNTH, 2, &data[0]); // read FIFO sample count
	...
	for (ii = 0; ii < packet_count; ii++)
	{
		...
		I2Cdev_readBytes(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_R_W, 12, &data[0]); // read data for averaging
    ...
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, XG_OFFS_USRH, data[0]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, XG_OFFS_USRL, data[1]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, YG_OFFS_USRH, data[2]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, YG_OFFS_USRL, data[3]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ZG_OFFS_USRH, data[4]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ZG_OFFS_USRL, data[5]);
    ...
    I2Cdev_readBytes(MPU9x50_USIC,MPU9250_ADDRESS, XA_OFFSET_H, 2, &data[0]); // Read factory accelerometer trim values
    accel_bias_reg[0] = (int16_t) ((int16_t)data[0] << 8) | data[1];
    I2Cdev_readBytes(MPU9x50_USIC,MPU9250_ADDRESS, YA_OFFSET_H, 2, &data[0]);
    accel_bias_reg[1] = (int16_t) ((int16_t)data[0] << 8) | data[1];
    I2Cdev_readBytes(MPU9x50_USIC,MPU9250_ADDRESS, ZA_OFFSET_H, 2, &data[0]);
    ...
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, XA_OFFSET_H, data[0]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, XA_OFFSET_L_TC, data[1]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, YA_OFFSET_H, data[2]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, YA_OFFSET_L_TC, data[3]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ZA_OFFSET_H, data[4]);
    I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ZA_OFFSET_L_TC, data[5]);
}
...
bool MPU9250_SelfTest(void) //
{
   ...
   I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, ACCEL_CONFIG, 0xF0); // Enable self test on all three axes and set accelerometer range to +/- 8 g
   I2Cdev_writeByte(MPU9x50_USIC,MPU9250_ADDRESS, GYRO_CONFIG,  0xE0); // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
   delay(250);  // Delay a while to let the device execute the self-test
   rawData[0] = I2Cdev_readByte(MPU9x50_USIC,MPU9250_ADDRESS, SELF_TEST_X); // X-axis self-test results
   rawData[1] = I2Cdev_readByte(MPU9x50_USIC,MPU9250_ADDRESS, SELF_TEST_Y); // Y-axis self-test results
   rawData[2] = I2Cdev_readByte(MPU9x50_USIC,MPU9250_ADDRESS, SELF_TEST_Z); // Z-axis self-test results
   rawData[3] = I2Cdev_readByte(MPU9x50_USIC,MPU9250_ADDRESS, SELF_TEST_A); // Mixed-axis self-test results
...
void MPU9X50_Ext_Int_ISR(void)
{
	...
	dcounter++;
		...
		mpuIntStatus = I2Cdev_readByte(MPU9x50_USIC,MPU9250_ADDRESS, INT_STATUS);
		//FIFO-Count
		I2Cdev_readBytes(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_COUNTH, 2, data);
		...
		{
			//Read from FIFO
			//if (I2Cdev_readBytes(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_R_W, 19, data))
			if (I2Cdev_readBytes(MPU9x50_USIC,MPU9250_ADDRESS, FIFO_R_W, 12, data))
			{
				...
				if (calib)
				{
					...
					if (calib_cnt > 512)
					{
						...
						I2Cdev_readBytes(MPU9x50_USIC, MPU9250_ADDRESS, XA_OFFSET_H, 2, data); // Read factory accelerometer trim values
						acc_bias_reg[0] = ((int16_t) ((int16_t)data[0] << 8) | data[1]) >> 1;
						I2Cdev_readBytes(MPU9x50_USIC, MPU9250_ADDRESS, YA_OFFSET_H, 2, data);
						acc_bias_reg[1] = ((int16_t) ((int16_t)data[0] << 8) | data[1]) >> 1;
						I2Cdev_readBytes(MPU9x50_USIC, MPU9250_ADDRESS, ZA_OFFSET_H, 2, data);
